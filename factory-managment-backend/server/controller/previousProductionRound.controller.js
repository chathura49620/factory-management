...
exports.create = (req,res)=>{
    ...
    previousProductionRoundDet
        ...
        .catch(err =>{
            res.status(500).send({
                message : err.message || "Error: Could Not Add Previous Production Round Details."
            });
        });
...
exports.find = (req,res) =>{
    ...
    if(req.query.id){
        ...
        previousProductionRoundDetails.findById(id)
        .then(data =>{
            if(!data){
                res.status(404).send({message:"Could Not Find Previous Production Round Details With ID" + id});
            }else{
                res.send(data);
            }
        })
        .catch(err =>{
            res.status(500).send({message:"Error While Retrieving Previous Production Round Details With ID" +id})
        })
    }else{
        previousProductionRoundDetails.find()
        .then(matCode => {
            res.send(matCode)
        })
        .catch(err =>{
            res.status(500).send({message:err.message || "Error: Cannot Retrieve Previous Production Round Details"})
        })
    }
}
...
exports.update = (req,res) => {
    ...
    previousProductionRoundDetails.findByIdAndUpdate(id, req.body,{useFindAndModify:false})
    .then(data =>{
        if(!data){
            res.status(400).send({message:"Previous Production Round Details Is Not Found"});
        }else{
            res.send(data);
        }
    })
...
exports.delete = (req,res) => {
    ...
    previousProductionRoundDetails.findByIdAndDelete(id)
    .then(data=>{
        if(!data){
            res.status(400).send({message:`Can Not Delete Previous Production Round Details With $(id).`});
        }
        else{
